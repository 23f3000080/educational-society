...
@admin_assignment_bp.route("/admin/assignments", methods=["GET"])
@token_required
@roles_required("admin")
def get_all_assignments_with_submissions(current_user):
    """
    ...
    try:
        ...
        for assignment in assignments:
            ...
            total_submissions = AssignmentSubmission.query.filter_by(
                assignment_id=assignment.id
            ).count()
            ...
            response.append({
                ...
                "students_submitted": total_submissions,

                "due_date": assignment.due_date.isoformat()
                if assignment.due_date else None,

                "created_at": assignment.created_at.isoformat()
                if assignment.created_at else None
...
@admin_assignment_bp.route("/assignments/<int:assignment_id>", methods=["GET"])
@token_required
@roles_required("admin")
def get_assignment_data(current_user, assignment_id):
    """
    Get complete details of a single assignment.
    """
    ...
    try:
        ...
        for question in questions:
            ...
            if question.question_type in ["mcq", "multiple_select"]:
                ...
                for option in options:
                    ...
                    if option.is_correct:
                        question_data["correct_answers"].append({
                            "option_id": option.id,
                            "option_text": option.option_text
                        })
            ...
            # Fill Blank
                ...
                blanks = FillBlankAnswer.query.filter_by(
                    question_id=question.id
                ).all()
        ...
        students_submitted = AssignmentSubmission.query.filter_by(
            assignment_id=assignment.id
        ).count()
...
@admin_assignment_bp.route("/assignments/<int:assignment_id>/submissions", methods=["GET"])
@token_required
@roles_required("admin")
def get_assignment_submissions(current_user, assignment_id):
    """
    Get all students who submitted an assignment.
    Marks are calculated dynamically.
    """
            ...
            for question in questions:
                ...
                if question.question_type == "mcq":
                    ...
                    if answer.selected_option_id:
                        ...
                        option = QuestionOption.query.get(
                ...
                # --------------------
                    ...
                    if answer and answer.text_answer:
                        ...
                        student_options = {
                            int(option.strip())
                            for option in answer.text_answer.split(",")
                            if option.strip()
                        }
                        ...
                        wrong_selected = student_options - correct_options

                        if not wrong_selected:

                            total_correct = len(correct_options)

                            if total_correct > 0:

                                marks_per_option = question.marks / total_correct

                                correct_selected = len(
                                    student_options & correct_options
                                )

                                obtained_marks += (
                                    correct_selected * marks_per_option
                                )
                ...
                # --------------------
                    ...
                    if answer.text_answer:
                        ...
                        for blank in blanks:
                            ...
                            if (
                                blank.correct_answer.strip().lower()
                                ==
...
@admin_assignment_bp.route(
    "/assignments/<int:assignment_id>/students/<int:student_id>",
    methods=["GET"]
)
@token_required
@roles_required("admin")
def get_student_submission(current_user, assignment_id, student_id):
        ...
        for question in questions:
            ...
            if question.question_type == "mcq":
                ...
                for option in options:
                    ...
                    if option.is_correct:
                        data["correct_answer"] = option.option_text

                if answer and answer.selected_option_id:

                    selected = QuestionOption.query.get(
                        answer.selected_option_id
                    )

                    if selected:

                        data["student_answer"] = selected.option_text
            ...
            # -----------------------------------
                ...
                if answer and answer.text_answer:
                    ...
                    selected_ids = {
                        int(x.strip())
                        for x in answer.text_answer.split(",")
                        if x.strip()
                    }
                ...
                for option in options:
                    ...
                    if selected:
                        selected_answers.append(option.option_text)

                    if option.is_correct:
                        correct_answers.append(option.option_text)

                data["student_answer"] = selected_answers
            ...
            # -----------------------------------
                ...
                if answer:
                    ...
                    data["student_answer"] = answer.text_answer

                    for blank in blanks:

                        if (
                            blank.correct_answer.lower().strip()
                            ==
                            answer.text_answer.lower().strip()
                        ):
